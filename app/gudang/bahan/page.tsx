...
export default function BahanPage() {
  const [bahan, setBahan] = useState<Bahan[]>(initialBahan);
  ...
  const stokMenipis = useMemo(
    () => bahan.filter((item) => item.stok <= item.minimum).length,
    [bahan]
  );
  ...
  function simpanTransaksi() {
    const item = bahan.find((row) => String(row.id) === selectedId);
    const qty = Number(jumlah);

    if (!item || !qty || qty <= 0) {
      alert("Pilih bahan dan isi jumlah transaksi.");
      return;
    }

    if (jenis === "Keluar" && qty > item.stok) {
    ...
    setBahan((prev) =>
      prev.map((row) =>
        row.id === item.id
          ? { ...row, stok: row.stok + (jenis === "Masuk" ? qty : -qty) }
          : row
      )
    );
      ...
      <div style={containerStyle}>
        ...
        <section style={sectionStyle}>
          <div style={headerStyle}>
            <div>
              <h1 style={titleStyle}>Bahan Makanan</h1>
              <p style={descriptionStyle}>Catat stok bahan makanan secara sederhana dan mandiri.</p>
            </div>
          ...
          {showForm && (
            <div style={formPanelStyle}>
              ...
              <button onClick={tambahBahan} style={primaryButtonStyle}>Simpan Bahan</button>
            </div>
          )}

          <div style={statsGridStyle}>
            <Stat label="Jenis Bahan" value={bahan.length} />
            <Stat label="Stok Menipis" value={stokMenipis} warning />
          </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={formGridStyle}>
              ...
              <Field label="Jenis"><select value={jenis} onChange={(e) => setJenis(e.target.value as "Masuk" | "Keluar")} style={inputStyle}><option>Masuk</option><option>Keluar</option></select></Field>
              <Field label="Jumlah"><input type="number" min="0" value={jumlah} onChange={(e) => setJumlah(e.target.value)} style={inputStyle} /></Field>
            </div>
            <button onClick={simpanTransaksi} style={primaryButtonStyle}>Simpan Transaksi</button>
          </div>

          <div style={tableWrapperStyle}>
            <h2 style={tableHeaderStyle}>Stok Saat Ini</h2>
            {bahan.length === 0 ? <div style={emptyStateStyle}>Belum ada bahan. Tambahkan bahan pertama.</div> : <table style={tableStyle}><thead><tr><th style={thStyle}>No</th><th style={thStyle}>Nama Bahan</th><th style={thStyle}>Kategori</th><th style={thStyle}>Stok</th><th style={thStyle}>Minimum</th></tr></thead><tbody>{bahan.map((item, index) => <tr key={item.id}><td style={tdStyle}>{index + 1}</td><td style={tdStyle}>{item.nama}</td><td style={tdStyle}>{item.kategori}</td><td style={tdStyle}>{item.stok} {item.satuan}</td><td style={tdStyle}>{item.minimum} {item.satuan}</td></tr>)}</tbody></table>}
          </div>

          <div style={tableWrapperStyle}>
            <h2 style={tableHeaderStyle}>Riwayat Transaksi</h2>
            {transaksi.length === 0 ? <div style={emptyStateStyle}>Belum ada transaksi.</div> : <table style={tableStyle}><thead><tr><th style={thStyle}>Tanggal</th><th style={thStyle}>Bahan</th><th style={thStyle}>Jenis</th><th style={thStyle}>Jumlah</th></tr></thead><tbody>{transaksi.map((item) => <tr key={item.id}><td style={tdStyle}>{item.tanggal}</td><td style={tdStyle}>{item.nama}</td><td style={tdStyle}>{item.jenis}</td><td style={tdStyle}>{item.jumlah} {item.satuan}</td></tr>)}</tbody></table>}
          </div>
        </section>
      </div>
    </main>
