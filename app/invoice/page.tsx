...
type SavedInvoice = {
  ...
  venueDate: string; venueTime: string; location: string; tax: number; paid: number; paidAfterEvent?: number;
...
export default function InvoicePage() {
  ...
  const [tax, setTax] = useState(0), [paid, setPaid] = useState(0), [paidAfterEvent, setPaidAfterEvent] = useState(0), [items, setItems] = useState<InvoiceItem[]>(defaultItems);
  ...
  const totalPaid = Number(paid || 0) + Number(paidAfterEvent || 0);
  const remaining = Math.max(total - totalPaid, 0);
  const isPaidOff = remaining <= 0 && total > 0;
  ...
  function openWhatsApp() { const digits = phone.replace(/\D/g, "").replace(/^0/, "62"); if (!digits) { setStatus("Isi nomor WhatsApp customer terlebih dahulu."); return; } const facilities = [karaokeFree ? "Karaoke Free" : "", liveMusic ? `Live Music Rp ${money(liveMusicPrice)}` : ""].filter(Boolean).join(", ") || "Tidak ada"; const message = `Halo ${customer || "Bapak/Ibu"}, invoice ${invoiceNo} dari Satu Restoe Pangandaran sudah dibuat. Fasilitas: ${facilities}. Total Rp ${money(total)}, uang muka Rp ${money(paid)}, bayar sehabis event Rp ${money(paidAfterEvent)}, sisa pembayaran Rp ${money(remaining)}${isPaidOff ? ", status LUNAS" : ""}.`; window.open(`https://wa.me/${digits}?text=${encodeURIComponent(message)}`, "_blank"); }
...
const styles = `
  * { box-sizing: border-box; }
  ...
  .payment-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; max-width: 760px; margin-top: 20px; } .action-row { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 22px; } .status { margin-top: 14px; padding: 10px 12px; border-radius: 8px; background: #edf8f4; color: #176b51; }
  ...
  .invoice-table { width: 100%; border-collapse: collapse; margin-top: 8mm; font-size: 10px; } .invoice-table th, .invoice-table td { border: 1px solid #69beeb; padding: 2.3mm 2mm; } .invoice-table th { background: #184f7e; color: white; text-align: center; } .invoice-table td:not(:nth-child(2)) { text-align: right; } .invoice-table td:first-child { text-align: center; } .invoice-table td:nth-child(2) { text-align: left; } .invoice-table tr td[colspan="4"] { text-align: right; } .invoice-table .highlight td { background: #e1f2fd; font-weight: bold; } .paid-stamp { display: inline-block; margin: 7mm 0 0 auto; padding: 3mm 8mm; border: 3px solid #16834b; border-radius: 2mm; color: #16834b; font-size: 20px; font-weight: 900; letter-spacing: 2px; transform: rotate(-7deg); }
  ...
  @media (max-width: 760px) { .page-shell { padding: 10px; } .heading-row, .saved-heading { align-items: flex-start; flex-direction: column; } .form-grid, .facility-grid, .payment-grid { grid-template-columns: repeat(2, 1fr); } .invoice-paper { padding: 7mm; } .paper-header { grid-template-columns: 38mm 1fr; gap: 4mm; } .paper-header img { width: 38mm; } .contact { grid-column: 2; text-align: left; font-size: 10px; } .title-row { flex-direction: column; } .invoice-meta { width: 100%; } .bottom-grid { gap: 4mm; } .saved-item { grid-template-columns: 1fr; gap: 4px; } }
